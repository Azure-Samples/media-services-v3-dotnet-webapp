...
builder.Services.AddStackExchangeRedisCache(options => builder.Configuration.GetSection("Redis").Bind(options));
...
class VideoAuthorizationFactory
{
    ...
    public VideoAuthorizationFactory(IHttpContextAccessor contextAccessor, Microsoft.Graph.GraphServiceClient graphServiceClient, IDistributedCache distributedCache)
    {
    ...
    public async Task<VideoAuthorizationValidator> CreateValidatorAsync()
    {
        ...
        var cacheKey = $"{user}_ObjectIds";

        var cachedObjectIds = await _distributedCache.GetStringAsync(cacheKey);

        var userObjectIds = default(HashSet<string>);

        if (cachedObjectIds != null)
        {
            userObjectIds = JsonSerializer.Deserialize<HashSet<string>>(cachedObjectIds)!;
        }
        else
        {
            userObjectIds = new HashSet<string> { user };
            ...
            await _distributedCache.SetStringAsync(
                cacheKey,
                JsonSerializer.Serialize(userObjectIds),
